...
const appPort = Number(process.env.AVW_VISUAL_APP_PORT || (18000 + (process.pid % 10000)));
const debugPort = Number(process.env.AVW_VISUAL_DEBUG_PORT || (28000 + (process.pid % 10000)));
const chrome = process.env.CHROME_BIN || "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";
const profile = join("/tmp", `avw-chrome-${runId}`);
const shotDir = join("/tmp", "avw-visual");
...
const unitSpritesShotPath = join(shotDir, "unit-sprites-cdp.png");
...
const watchdog = setTimeout(() => {
  console.error(`[avw-web] visual check timed out on appPort=${appPort} debugPort=${debugPort}\n${browserOutput}`);
  stopProcess(browser);
  stopProcess(server);
  process.exit(1);
}, 120000);
try {
  ...
  const menuSession = await openScenario(cdp, {
    ...
    url: `http://127.0.0.1:${appPort}/?king=ansem&rival=wynn&pressure=rush&arena=creek&case=menu`,
  });
  ...
  assert(state.selectedKing === "ansem", `URL king param did not apply: ${JSON.stringify(state)}`);
  assert(state.rivalKing === "wynn", `URL rival param did not apply: ${JSON.stringify(state)}`);
  ...
  const matchSession = await openScenario(cdp, {
    viewport: { width: 1280, height: 720, deviceScaleFactor: 1, mobile: false },
    url: `http://127.0.0.1:${appPort}/?king=ansem&rival=wynn&pressure=siege&arena=ruins&start=1&selectionSummaryPreview=1&case=match`,
  });
  ...
  assert(matchState.playerKing === "ansem", `start URL king did not reach match: ${JSON.stringify(matchState)}`);
  assert(matchState.rivalKing === "wynn", `start URL rival did not reach match: ${JSON.stringify(matchState)}`);
  ...
  const unitSpritesSession = await openScenario(cdp, {
    viewport: { width: 1280, height: 720, deviceScaleFactor: 1, mobile: false },
    url: `http://127.0.0.1:${appPort}/?king=ansem&rival=wynn&pressure=standard&arena=meadow&start=1&unitSpritePreview=1&case=unit-sprites`,
  });
  const unitSpritesState = await waitForUnitSpritePreview(cdp, unitSpritesSession);
  assert(unitSpritesState.playerKing === "ansem", `unit sprite preview did not force player king: ${JSON.stringify(unitSpritesState)}`);
  assert(Number(unitSpritesState.unitSpriteUnits) === 10, `unit sprite preview did not spawn all custom role units: ${JSON.stringify(unitSpritesState)}`);
  assert(Array.isArray(unitSpritesState.unitSpriteKings) && ["ansem", "wynn"].every((king) => unitSpritesState.unitSpriteKings.includes(king)), `unit sprite preview missing a king: ${JSON.stringify(unitSpritesState)}`);
  assert(Array.isArray(unitSpritesState.unitSpriteKinds) && ["villager", "swordsman", "archer", "lancer", "siege"].every((kind) => unitSpritesState.unitSpriteKinds.includes(kind)), `unit sprite preview missing a role: ${JSON.stringify(unitSpritesState)}`);
  await captureNonTrivialScreenshot(cdp, unitSpritesSession, unitSpritesShotPath);

  const resultSession = await openScenario(cdp, {
    viewport: { width: 1280, height: 720, deviceScaleFactor: 1, mobile: false },
    url: `http://127.0.0.1:${appPort}/?king=ansem&rival=wynn&pressure=rush&arena=creek&start=1&webResultSmoke=1&case=result`,
  });
  ...
  assert(resultState.leaderboardSubmittedKing === "ansem" && resultState.leaderboardSubmittedRival === "wynn" && resultState.leaderboardSubmittedResult === "won", `leaderboard submitted unexpected row: ${JSON.stringify(resultState)}`);
  ...
  const mobileMenuSession = await openScenario(cdp, {
    ...
    url: `http://127.0.0.1:${appPort}/?king=wynn&rival=ansem&pressure=standard&arena=pond&case=mobile-menu`,
  });
  const mobileMenuState = await waitForGodotScene(cdp, mobileMenuSession, "menu");
  assert(mobileMenuState.selectedKing === "wynn", `mobile URL king param did not apply: ${JSON.stringify(mobileMenuState)}`);
  assert(mobileMenuState.rivalKing === "ansem", `mobile URL rival param did not apply: ${JSON.stringify(mobileMenuState)}`);
  ...
  const portraitMenuSession = await openScenario(cdp, {
    ...
    url: `http://127.0.0.1:${appPort}/?king=ansem&rival=wynn&pressure=siege&arena=mesa&case=portrait-menu`,
  });
  const portraitMenuState = await waitForGodotScene(cdp, portraitMenuSession, "menu");
  assert(portraitMenuState.selectedKing === "ansem", `portrait menu URL king param did not apply: ${JSON.stringify(portraitMenuState)}`);
  assert(portraitMenuState.rivalKing === "wynn", `portrait menu URL rival param did not apply: ${JSON.stringify(portraitMenuState)}`);
  ...
  const mobileMatchSession = await openScenario(cdp, {
    viewport: { width: 844, height: 390, deviceScaleFactor: 2, mobile: true },
    url: `http://127.0.0.1:${appPort}/?king=wynn&rival=ansem&pressure=rush&arena=festival&start=1&case=mobile-match`,
  });
  const mobileMatchState = await waitForGodotScene(cdp, mobileMatchSession, "main");
  assert(mobileMatchState.playerKing === "wynn", `mobile start URL king did not reach match: ${JSON.stringify(mobileMatchState)}`);
  assert(mobileMatchState.rivalKing === "ansem", `mobile start URL rival did not reach match: ${JSON.stringify(mobileMatchState)}`);
  ...
  const portraitMatchSession = await openScenario(cdp, {
    viewport: { width: 390, height: 844, deviceScaleFactor: 2, mobile: true },
    url: `http://127.0.0.1:${appPort}/?king=ansem&rival=wynn&pressure=standard&arena=meadow&start=1&case=portrait-match`,
  });
  const portraitMatchState = await waitForGodotScene(cdp, portraitMatchSession, "main");
  assert(portraitMatchState.playerKing === "ansem", `portrait start URL king did not reach match: ${JSON.stringify(portraitMatchState)}`);
  assert(portraitMatchState.rivalKing === "wynn", `portrait start URL rival did not reach match: ${JSON.stringify(portraitMatchState)}`);
  ...
  const fractionalDprSession = await openScenario(cdp, {
    ...
    url: `http://127.0.0.1:${appPort}/?king=ansem&rival=wynn&pressure=standard&arena=meadow&start=1&unitSpritePreview=1&case=fractional-dpr`,
  });
  await waitForUnitSpritePreview(cdp, fractionalDprSession);
  ...
  console.log(`[avw-web] exported app reached desktop, unit sprite preview, fractional DPR, landscape mobile, and portrait mobile menu/match/result in Chrome; screenshots ${menuShotPath}, ${matchShotPath}, ${unitSpritesShotPath}, ${fractionalDprShotPath}, ${resultShotPath}, ${mobileMenuShotPath}, ${portraitMenuShotPath}, ${mobileMatchShotPath}, ${portraitMatchShotPath}`);
} finally {
...
async function navigate(cdp, sessionId, url) {
  await cdp.send("Runtime.evaluate", {
    expression: "window.__avwState = {}; window.__avwEvents = [];",
    awaitPromise: true,
  }, sessionId).catch(() => {});
...
async function waitForGodotScene(cdp, sessionId, scene) {
  ...
  let last = {};
  while (Date.now() < deadline) {
    last = await evaluate(cdp, sessionId, "window.__avwState || {}");
...
async function waitForUnitSpritePreview(cdp, sessionId) {
  const deadline = Date.now() + 30000;
  let last = {};
  while (Date.now() < deadline) {
    last = await evaluate(cdp, sessionId, "window.__avwState || {}");
    if (last && last.scene === "main" && last.unitSpritePreview === true) return last;
    await delay(500);
  }
