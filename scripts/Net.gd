...
func _ready() -> void:
	if OS.has_feature("web"):
		var injected = JavaScriptBridge.eval("window.AVW_ROOM_RELAY || ''", true)
...
func launch(arena: String, pressure: String) -> void:
	...
	send_msg({"type": "match-config", "arena": arena, "pressure": pressure, "host_king": Game.player_king})
	launch_match.emit({"arena": arena, "pressure": pressure, "host_king": Game.player_king})
	...
	match String(msg.get("type", "")):
		...
		"match-config":
			launch_match.emit({
				...
				"host_king": String(msg.get("host_king", "ansem")),
			})
		_:
